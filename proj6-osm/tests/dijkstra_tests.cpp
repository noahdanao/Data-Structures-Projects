...
unordered_map<long long, Coordinates> COORDINATES;

void fillUicGraph() {
  if (UIC_GRAPH.numVertices())
    return;

  vector<BuildingInfo> buildings;
  ifstream input("data/uic-fa25.osm.json");
  buildGraph(input, UIC_GRAPH, buildings, COORDINATES);
...
TEST(Dijkstra, Full) {
  ...
  vector<long long> expectedArcToSh = {
      664275388,   13147536806, 12421562289, 7282826590,  7282826594,
      8926492677,  12421562286, 8931833681,  12057116096, 1308089676,
      10790261682, 10790261681, 1308089711,  12057116097, 7282826602,
      11174876827, 464345395,   11174876819, 11257936376, 462010749,
      9870872078,  462010764,   151676521};
  vector<long long> expectedSeoToSh = {
      151960667,  1647971930,  462010746,  12108530536, 1645121274, 12923649673,
...
TEST(Dijkstra, FullSkipNodes) {
  ...
  vector<long long> expectedArcToLcb = {
      ...
      10790261682, 10790261681, 1308089711,  12057116097, 7282826602,
      11174876827, 464345395,   11174876819, 11257936376, 462010749,
      9870872078,  462010764,   462010765,   9870872105,  462010742,
      9870872103,  12108530540, 151672203};
  vector<long long> expectedErfToLcb = {
      151960677, 1647971980, 1647971942, 1647971957,  462014176, 9870872090,
      462010748, 9862302460, 462010745,  10930768589, 151672203};
